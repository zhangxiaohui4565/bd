...
/**
  ...
  * spark-submit --master yarn-client --class com.gupao.bigdata.spark.demo.ComputePV ./spark-demo-1.0-SNAPSHOT-uber.jar --input_path /user/root/bill/flume/events --save_mode HDFS --output_path /user/root/bill/nginx/output_pv
  * spark-submit --master yarn-client --class com.gupao.bigdata.spark.demo.ComputePV ./spark-demo-1.0-SNAPSHOT-uber.jar --input_path /user/root/bill/flume/events --save_mode DB
  * */
object ComputePV {
  ...
  def computePV(textRDD: RDD[String]): RDD[(String, Long)] = {
    ...
    val result = splitTextFileRDD
      ...
      .map(log => ((new java.net.URL(log(1))).getPath, 1L))
      .reduceByKey(_ + _)
    result
  }
  ...
  def saveToMysql(sc: SparkContext, result: RDD[(String, Long)]): Unit = {
    ...
    result
      .toDF("url", "count")
      .write.mode(SaveMode.Append).jdbc("jdbc:mysql://gp-bd-master01/web_data", "web_pv", props)
  }
